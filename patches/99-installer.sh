...
{
	echo ""
	echo "Crunch installer: install chromeos on device or create disk image from the running environment."
...
menuentry "ChromeOS (boot from disk image)" {
	...
	linux (loop,gpt7)/kernel boot=local noresume noswap loglevel=7 disablevmx=off \\\\
		...
		lsm.module_locking=0 add_efi_memmap i915.modeset=1 tpm_tis.force=1 tpm_tis.interrupts=0
