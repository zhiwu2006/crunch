...
if mountpoint -q ~/crunch-mount/tmp; then umount ~/crunch-mount/tmp; fi
if mountpoint -q ~/crunch-mount; then umount ~/crunch-mount; fi
if [ -d ~/crunch-mount ]; then rm -r ~/crunch-mount; fi

usage()
{
	echo ""
	echo "Crunch installer: install chromeos on device or create disk image from the crunch framework."
...
if [[ $device = 1 ]]; then
	...
	for (( i=1; i<=12; i++ )); do
		...
		case $i in
			...
			;;
			7)
			mkdir -p ~/crunch-mount
			mount rootc.img ~/crunch-mount
			vbutil_kernel --get-vmlinuz "$loopdevice"p4 --vmlinuz-out ~/crunch-mount/kernel
			mkdir ~/crunch-mount/initramfs
			gunzip -c ~/crunch-mount/initramfs.img > ~/crunch-mount/initramfs/initramfs.cpio
			rm ~/crunch-mount/initramfs.img
			(cd ~/crunch-mount/initramfs && cpio -i -H newc < initramfs.cpio)
			rm ~/crunch-mount/initramfs/initramfs.cpio
			mkdir ~/crunch-mount/tmp
			mount -o,ro "$loopdevice"p3 ~/crunch-mount/tmp
			rm -r ~/crunch-mount/initramfs/lib/*
			cp -r ~/crunch-mount/tmp/lib/firmware ~/crunch-mount/initramfs/lib/
			umount ~/crunch-mount/tmp
			rm -r ~/crunch-mount/tmp
			(cd ~/crunch-mount/initramfs && find . | cpio -o -H newc | gzip > ~/crunch-mount/initramfs.img)
			rm -r ~/crunch-mount/initramfs
			umount ~/crunch-mount
			rm -r ~/crunch-mount
	...
	for (( i=1; i<=12; i++ )); do
		...
		case $i in
			...
			mkdir ~/crunch-mount
			mount rootc.img ~/crunch-mount
			dd if="$source" ibs=512 count=$(cgpt show -i 4 -s "$source") skip=$(cgpt show -i 4 -b "$source") status=none | pv | dd of=~/crunch-mount/bootimg
			vbutil_kernel --get-vmlinuz ~/crunch-mount/bootimg --vmlinuz-out ~/crunch-mount/kernel
			rm ~/crunch-mount/bootimg
			mkdir ~/crunch-mount/initramfs
			gunzip -c ~/crunch-mount/initramfs.img > ~/crunch-mount/initramfs/initramfs.cpio
			rm ~/crunch-mount/initramfs.img
			(cd ~/crunch-mount/initramfs && cpio -i -H newc < initramfs.cpio)
			rm ~/crunch-mount/initramfs/initramfs.cpio
			mkdir ~/crunch-mount/tmp
			dd if="$source" ibs=512 count=$(cgpt show -i 3 -s "$source") skip=$(cgpt show -i 3 -b "$source") status=none | pv | dd of=./roota.img obs=1M conv=notrunc status=none
			mount -o,ro ./roota.img ~/crunch-mount/tmp
			rm -r ~/crunch-mount/initramfs/lib/*
			cp -r ~/crunch-mount/tmp/lib/firmware ~/crunch-mount/initramfs/lib/
			umount ~/crunch-mount/tmp
			rm ./roota.img
...
- A crunch usb flashdrive grub config file (then boot from usb and choose boot from disk image in the menu),
...
menuentry "ChromeOS (boot from disk image)" {
	...
	linux (loop,gpt7)/kernel boot=local noresume noswap loglevel=7 disablevmx=off \\
		...
		lsm.module_locking=0 add_efi_memmap i915.modeset=1 hung_task_panic=0
